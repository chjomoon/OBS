...
<body>
	<%
        ...
        if(session.getAttribute("userID")!=null){
			...
			shoppingList.getOrderList(userID);
        }
		request.setCharacterEncoding("UTF-8");
		int cartID = 0;
    ...
    <div class="col-lg-12 order-lg-2 mb-4">
          ...
          <ul class="list-group mb-3">
              ...
              <%
				try{
					if(shoppingList.getOrderList(userID) == null) {
						out.println("Empty");
					}else{
						for(int i = 0 ; i < shoppingList.orderList.size(); i++){
						Order cart = shoppingList.orderList.get(i);
						System.out.println(cart.getIsbn());
						//Books shoppingBook = shoppingList.getBookList(cart.getIsbn()).get(0);
				%>
            <div class = "histlist">
              <li class="list-group-item d-flex justify-content-between lh-condensed"> 
                <h8 class="price"><% out.println(cart.getTransactionID()); %></h8>
                <h8 class="price"><% out.println(cart.getOrderDate()); %></h8>
                <h8 class="price">Pending<%  %></h8>
                  <strong class="price">$ <% out.println(cart.getTotalPrice()); %></strong>
              </li>
                      
                <small class="text-muted">Ordered Books: <% //out.println(shoppingBook.getTitle()); %></small>
                <br>
                <small class="text-muted">payment method: credit card</small>
                <br>
                <small class="text-muted">Shipping Address : <% out.println(userDAO.findInfo(userID, "userAddr"));%> ,GA,30609</small>
                <br>
                <small class="text-muted">Billing address: : <% out.println(userDAO.findInfo(userID, "userAddr"));%> ,GA,30609</small>
				<%
						}	
					}
				}catch(Exception e){
